...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS password_set_at TIMESTAMPTZ NULL;

UPDATE public.profiles p
SET password_set_at = COALESCE(u.last_sign_in_at, p.created_at)
FROM auth.users u
WHERE u.id = p.id
...
CREATE OR REPLACE FUNCTION public.get_admin_user_stats()
 RETURNS TABLE(id uuid, name text, email text, created_at timestamp with time zone, friend_count integer, active_tickets integer, active_wanted integer, messages_sent integer, messages_received integer, last_sign_in_at timestamp with time zone, has_password boolean, password_set_at timestamp with time zone)
 LANGUAGE plpgsql
