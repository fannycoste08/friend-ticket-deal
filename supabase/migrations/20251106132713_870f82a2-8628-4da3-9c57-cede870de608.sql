-- Add authorization check to get_mutual_friends function
...
AS $$
BEGIN
  -- Security check: Only allow users to query their own mutual friends
  IF user_a != auth.uid() AND user_b != auth.uid() THEN
    RAISE EXCEPTION 'Unauthorized: You can only view mutual friends for your own connections';
  END IF;
