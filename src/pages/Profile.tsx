...
const Profile = () => {
  ...
  const loadWantedTickets = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('wanted_tickets').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    });
    if (error) {
  ...
  const handleDeleteWantedTicket = async (id: string) => {
    const {
      error
    } = await supabase.from('wanted_tickets').delete().eq('id', id);
    if (error) {
  ...
  const loadFriends = async () => {
    ...
    } = await supabase.from('friendships').select('user_id, friend_id').eq('status', 'accepted').or(`user_id.eq.${user.id},friend_id.eq.${user.id}`);
    if (friendshipsError) {
  ...
  const handleDeleteFriend = async () => {
    ...
    const {
      error
    } = await supabase.from('friendships').delete().or(`user_id.eq.${user.id},friend_id.eq.${user.id}`).or(`user_id.eq.${friendToDelete.id},friend_id.eq.${friendToDelete.id}`);
    if (error) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-b from-background to-secondary/30 flex items-center justify-center">
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background to-secondary/30 pb-20">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text mb-2 text-blue-600">
            Mi Perfil
          </h1>
          <p className="text-muted-foreground">Información de tu cuenta</p>
        </div>

        <Card className="p-4 sm:p-8 mb-6" style={{
        ...
        <div className="mt-6">
          ...
          {loadingFriends ? <div className="text-center py-12">
              <p className="text-muted-foreground">Cargando tus amigos...</p>
            </div> : friends.length === 0 ? <div className="text-center py-12 bg-secondary/30 rounded-lg">
              ...
              {friends.map(friend => <Card key={friend.id} className="p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setFriendToDelete(friend)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
        ...
        <div className="mt-6">
          ...
          {loadingTickets ? <div className="text-center py-12">
              <p className="text-muted-foreground">Cargando tus entradas...</p>
            </div> : tickets.length === 0 ? <div className="text-center py-12 bg-secondary/30 rounded-lg">
              ...
              {tickets.map(ticket => <MyTicketCard key={ticket.id} ticket={ticket} onEdit={() => setEditingTicket(ticket)} onDelete={() => handleDeleteTicket(ticket.id)} onMarkAsSold={() => handleMarkAsSold(ticket.id)} />)}
            </div>}
        </div>
        ...
        <div className="mt-6">
          ...
          {loadingWanted ? <div className="text-center py-12">
              <p className="text-muted-foreground">Cargando tus búsquedas...</p>
            </div> : wantedTickets.length === 0 ? <div className="text-center py-12 bg-accent/5 rounded-lg border border-accent/20">
              ...
              {wantedTickets.map(ticket => <MyWantedTicketCard key={ticket.id} ticket={ticket} onEdit={() => setEditingWantedTicket(ticket)} onDelete={() => handleDeleteWantedTicket(ticket.id)} />)}
            </div>}
        </div>
          ...
          boxShadow: 'var(--shadow-card)'
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                <Switch id="email-notifications" checked={emailNotificationsEnabled} onCheckedChange={toggleEmailNotifications} />
              </div>
            </div>
          </Card>
        </div>
          ...
          boxShadow: 'var(--shadow-card)'
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex items-center flex-shrink-0">
                <Button variant="destructive" onClick={() => setShowDeleteAccountDialog(true)} className="bg-red-600 hover:bg-red-700 text-white w-full sm:w-auto">
        ...
        {editingTicket && <TicketForm editTicket={editingTicket} onSuccess={() => {
        loadTickets();
        setEditingTicket(undefined);
      }} />}

        {editingWantedTicket && <WantedTicketForm editTicket={editingWantedTicket} onSuccess={() => {
        loadWantedTickets();
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-4">
              ...
              <div className="pt-4">
                ...
                <Input id="confirm-delete" value={deleteConfirmText} onChange={e => setDeleteConfirmText(e.target.value)} placeholder="ELIMINAR" className="mt-2" disabled={isDeletingAccount} />
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeletingAccount}>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteAccount} disabled={deleteConfirmText !== 'ELIMINAR' || isDeletingAccount} className="bg-red-600 hover:bg-red-700 text-white disabled:opacity-50">
